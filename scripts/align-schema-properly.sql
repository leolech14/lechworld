...
-- 1. First rename loyalty_programs to airlines
...
-- 10. Move status to status_level if exists
DO $$ 
BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.columns 
             WHERE table_name = 'member_programs' AND column_name = 'status') THEN
    UPDATE member_programs SET status_level = status WHERE status_level IS NULL;
    ALTER TABLE member_programs DROP COLUMN status;
  END IF;
END $$;
...
-- 14. Create missing tables for other features (if they don't exist)
